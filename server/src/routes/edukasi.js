...
const normalizeRow = (row) => {
  ...
  const images = Array.isArray(row.images)
    ? row.images.filter((s) => typeof s === "string" && s.trim())
    : [];
...
router.get("/", async (req, res) => {
  ...
  try {
    const [rawItems, total] = await Promise.all([
      prisma.education.findMany({
        ...
        select: {
          ...
          images: true,
          excerpt: true,
          // content sengaja tidak dipilih untuk list (hemat payload)
          popularity: true,
          viewCount: true,
          shareCount: true,
    ...
    console.error("GET /edukasi error", e);
    res.status(500).json({ error: "Gagal memuat data edukasi" });
  }
});

/** GET /api/edukasi/id/:id (untuk admin edit) */
router.get("/id/:id", async (req, res) => {
  ...
  try {
    ...
    console.error("GET /edukasi/id/:id error", e);
    res.status(500).json({ error: "Gagal memuat detail edukasi" });
  }
});
...
router.get("/:slug", async (req, res) => {
  ...
  try {
    ...
    console.error("GET /edukasi/:slug error", e);
    res.status(500).json({ error: "Gagal memuat detail edukasi" });
  }
});

/* ===================== Mutasi (SUPER_ADMIN saja) ===================== */
/** POST /api/edukasi */
router.post("/", requireAuth, allowRoles("SUPER_ADMIN"), async (req, res) => {
  try {
    ...
    const must = { title, slug, date, content };
    ...
    if (imagesArr.length > MAX_IMAGES) {
      return res.status(400).json({ error: `Maksimal ${MAX_IMAGES} gambar.` });
    }
    ...
    console.error("POST /edukasi error", e);
    if (e?.code === "P2002") return res.status(409).json({ error: "Slug sudah digunakan" });
    res.status(500).json({ error: "Gagal menyimpan edukasi" });
  }
});
...
router.put("/:id", requireAuth, allowRoles("SUPER_ADMIN"), async (req, res) => {
  ...
  try {
    ...
    console.error("PUT /edukasi/:id error", e);
    if (e?.code === "P2002") return res.status(409).json({ error: "Slug sudah digunakan" });
    if (e?.code === "P2025") return res.status(404).json({ error: "Edukasi tidak ditemukan" });
...
router.delete("/:id", requireAuth, allowRoles("SUPER_ADMIN"), async (req, res) => {
  ...
  try {
    ...
    console.error("DELETE /edukasi/:id error", e);
    if (e?.code === "P2025") return res.status(404).json({ error: "Edukasi tidak ditemukan" });
...
/* ===================== Counters ===================== */
/** POST /api/edukasi/:slug/view (publik) */
router.post("/:slug/view", async (req, res) => {
  ...
  try {
    ...
    console.error("POST /edukasi/:slug/view error", e);
    res.status(500).json({ error: "Gagal update view" });
  }
});

/**
 * POST /api/edukasi/:slug/share (publik)
 * - kalau user login (req.user.uid ada), simpan sharedByUserId
 * - kalau tidak login, tetap increment shareCount
 */
router.post("/:slug/share", async (req, res) => {
  ...
  try {
    ...
    if (e?.code === "P2025") return res.status(404).json({ error: "Not found" });
    console.error("POST /edukasi/:slug/share error", e);
