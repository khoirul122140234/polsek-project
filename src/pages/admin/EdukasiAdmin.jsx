// src/pages/admin/EdukasiAdmin.jsx
...
import {
  ...
  ArrowLeft, ArrowRight, ImagePlus, XCircle
} from "lucide-react";
...
yearOptions.unshift(""); // "" = semua tahun
...
/* =========================== Form Edukasi =========================== */
function EdukasiForm({ initial, onSubmit, onCancel, submitting }) {
  ...
  // ✅ 1 klik: ringkasan otomatis dari konten
  const autoExcerpt = () => {
  ...
  const handleUpload = async (ev) => {
    ...
    try {
      ...
      /**
       * ✅ Upload SEKALI (multi) agar lebih cepat
       * backend /uploads/image mengembalikan:
       * - url (pertama)
       * - items: [{ url, path, filename, absoluteUrl }]
       */
      const res = await uploadImage(slice);

      const urlsFromItems = Array.isArray(res?.items)
        ? res.items.map((x) => x?.url).filter(Boolean)
        : [];

      // fallback kompat: jika items tidak ada, pakai res.url
      const uploaded = urlsFromItems.length ? urlsFromItems : (res?.url ? [res.url] : []);

      const next = [...(form.images || []), ...uploaded].slice(0, MAX_IMAGES);
      setField("images", next);

      // image utama = index 0
      if (!form.image && next.length > 0) setField("image", next[0]);
  ...
  const removeImageAt = (idx) => {
    ...
    setRev(Date.now()); // ✅ cache bust juga saat hapus
    if (fileRef.current) fileRef.current.value = "";
  };
  ...
  const count = form.images?.length || 0;
    ...
    <form onSubmit={submit} className="space-y-4">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div className="flex flex-col">
          ...
          {/* ✅ responsif: stack tombol Auto di mobile */}
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            <input
              ...
              placeholder="slug-edukasi"
              disabled={submitting}
            />
            <button
              type="button"
              onClick={autoSlug}
              className="w-full shrink-0 rounded-lg border border-gray-300 px-3 py-2 text-xs hover:bg-gray-50 sm:w-auto"
              disabled={submitting}
            >
              Auto
            </button>
        ...
        <div className="sm:col-span-2">
          ...
          {/* ✅ responsif: tombol + label stack di mobile */}
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
            <input
              ...
              disabled={submitting || uploading || atLimit}
              className="hidden"
            />
            <button
              type="button"
              ...
              disabled={submitting || uploading || atLimit}
              className={cn(
                "inline-flex w-full items-center justify-center gap-2 rounded-lg border border-gray-300 px-3 py-2 text-sm hover:bg-gray-50 sm:w-auto sm:justify-start",
                (submitting || uploading || atLimit) && "opacity-60 cursor-not-allowed"
              )}
            >
              <ImagePlus className="h-4 w-4" /> Tambah Gambar
            </button>
            <span className="text-xs text-gray-500">
              {uploading ? "Mengunggah…" : `Terpilih ${count} / ${MAX_IMAGES}`}
            </span>
          </div>
              ...
              {form.images.map((src, idx) => (
                <div key={`${src}-${idx}`} className="relative overflow-hidden rounded-lg ring-1 ring-gray-200">
                  ...
                  <div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-black/40 px-1 py-1">
                    <div className="text-[10px] text-white px-1 rounded bg-black/40">
                      {idx + 1}/{count}
                    </div>
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        ...
                        className={cn(
                          "rounded p-1 text-white/90 hover:bg-white/10",
                          idx === (form.images.length - 1) && "opacity-40 cursor-not-allowed"
                        )}
                        title="Geser kanan"
                      >
        ...
        <div className="sm:col-span-2">
          {/* ✅ tombol klik otomatis ringkasan */}
          ...
          <textarea
            ...
            placeholder="Ringkasan singkat edukasi… (boleh dikosongkan)"
            disabled={submitting}
          />
        ...
        <div className="sm:col-span-2">
          ...
          <textarea
            ...
            placeholder="Konten lengkap edukasi…"
            disabled={submitting}
          />
...
export default function EdukasiAdmin() {
  ...
  const [sort, setSort] = useState("terbaru"); // "terbaru" | "populer"
  ...
  // indikator loading tombol Edit baris tertentu
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const path = `/edukasi${params.toString() ? `?${params}` : ""}`;
        ...
        console.error(e);
        if (alive) setErr(e?.message || "Gagal memuat data edukasi");
      } finally {
  ...
  // Filter & pagination (client-side fallback)
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      const textOk = !q
        ? true
        : [r.title, r.slug, r.excerpt].filter(Boolean).some((s) => String(s).toLowerCase().includes(q));
  ...
  // === fetch detail saat klik Edit ===
  const openEdit = async (row) => {
    try {
      setDetailLoadingId(row.id);
      const detail = await get(`/edukasi/id/${row.id}`);
      setEditing(detail);
      setOpenForm(true);
    } catch (e) {
      console.error(e);
      alert(e?.message || "Gagal memuat detail edukasi untuk diedit.");
    } finally {
  ...
  const handleSubmit = async (payload) => {
    try {
      ...
      if (editing && editing.id) {
        const updated = await put(`/edukasi/${editing.id}`, payload);
        setRows((prev) => prev.map((r) => (r.id === editing.id ? updated : r)));
      } else {
        const created = await post("/edukasi", payload);
  ...
  const confirmDelete = async () => {
    ...
    try {
      setDeleting(true);
      await del(`/edukasi/${targetDelete.id}`);
      ...
      <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 flex w-full flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div className="min-w-0">
            <h1 className="text-2xl font-extrabold tracking-tight sm:text-3xl">Kelola Edukasi</h1>
            <p className="mt-1 text-sm text-gray-600">
              CRUD edukasi: daftar, tambah, ubah, hapus, upload gambar (maks {MAX_IMAGES}), dan pantau metrik.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 sm:justify-end">
            ...
            >
              <Plus className="h-4 w-4" /> Tambah Edukasi
            </button>
          </div>
        </div>
        ...
        <div className="mb-4 w-full">
          {/* ✅ responsif: grid bertahap (1 -> 2 -> 4 -> 8) */}
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-8">
            ...
            <div className="lg:col-span-1">
              <select
                value={month}
                onChange={(e) => { setMonth(e.target.value); resetPagination(); }}
            ...
            <div className="lg:col-span-1">
              <select
                value={year}
                onChange={(e) => { setYear(e.target.value); resetPagination(); }}
                className="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-black/20"
            ...
            {/* Page size */}
            <div className="flex items-center justify-between gap-2 sm:justify-end lg:col-span-2">
        ...
        <div className="space-y-3 sm:hidden">
          {loading ? (
            ...
            paged.map((r) => {
              const firstImg = (r.images?.[0]) || r.image;
              const extra = Math.max(0, (r.images?.length || 0) - 1);
              ...
              return (
                <div key={r.id} className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                  <div className="flex gap-3">
                    <div className="relative h-16 w-28 shrink-0 overflow-hidden rounded-xl ring-1 ring-gray-200">
                      <img
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 font-mono text-xs text-gray-600 whitespace-normal break-words overflow-wrap-anywhere">
                        {r.slug}
                      </div>

                      <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
                        <span className="rounded-full bg-gray-100 px-2.5 py-1 text-gray-800 ring-1 ring-gray-200">
                          Tgl: <span className="font-semibold">{formatDate(r.date)}</span>
                        </span>
                        <span className="rounded-full bg-gray-100 px-2.5 py-1 text-gray-800 ring-1 ring-gray-200">
                          Pop: <span className="font-semibold">{r.popularity ?? 0}</span>
                        </span>
                        <span className="rounded-full bg-gray-100 px-2.5 py-1 text-gray-800 ring-1 ring-gray-200">
                          Dilihat: <span className="font-semibold">{r.viewCount ?? 0}x</span>
                        </span>
                        <span className="rounded-full bg-yellow-100 px-2.5 py-1 text-yellow-900 ring-1 ring-yellow-200">
                          Share: <span className="font-semibold">{r.shareCount ?? 0}x</span>
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  {r.excerpt ? (
                    <div className="mt-3 text-sm text-gray-700 whitespace-normal break-words overflow-wrap-anywhere">
                      {r.excerpt}
                    </div>
                  ) : null}
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2">
                    ...
                    <button
                      onClick={() => { setTargetDelete(r); setOpenConfirm(true); }}
        ...
        <div className="hidden sm:block">
          <div className="w-full overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
            <div className="overflow-x-auto">
              {/* ✅ table-layout fixed + colgroup agar isi panjang turun ke bawah, bukan melebar ke kanan */}
              <table className="min-w-full text-left text-sm" style={{ tableLayout: "fixed" }}>
                <colgroup>
                  <col style={{ width: "140px" }} /> {/* gambar */}
                  <col style={{ width: "240px" }} /> {/* judul */}
                  <col style={{ width: "220px" }} /> {/* slug */}
                  <col style={{ width: "120px" }} /> {/* tanggal */}
                  <col style={{ width: "110px" }} /> {/* popularitas */}
                  <col style={{ width: "110px" }} /> {/* dilihat */}
                  <col style={{ width: "120px" }} /> {/* dibagikan */}
                  <col style={{ width: "auto" }} /> {/* ringkasan */}
                ...
                <tbody>
                  {loading ? (
                    <tr><td colSpan={9} className="px-4 py-10 text-center text-gray-500">Memuat…</td></tr>
                  ) : paged.length === 0 ? (
                    <tr><td colSpan={9} className="px-4 py-10 text-center text-gray-500">Tidak ada data.</td></tr>
                  ) : paged.map((r) => {
                    const firstImg = (r.images?.[0]) || r.image;
                    const extra = Math.max(0, (r.images?.length || 0) - 1);
                    const isLoadingThis = detailLoadingId === r.id;
                    ...
                    return (
                      <tr key={r.id} className="border-t align-top">
                        ...
                        <td className="px-4 py-3">
                          <div className="line-clamp-2 text-gray-700 whitespace-normal break-words overflow-wrap-anywhere">
                            {r.excerpt}
                          </div>
                        </td>
        ...
        <Modal
          ...
          title={editing ? "Edit Edukasi" : "Tambah Edukasi"}
        >
          <EdukasiForm
            ...
            initial={
              editing
                ? {
                    ...
                    image: editing.image || "",
                    images: Array.isArray(editing.images) && editing.images.length
        ...
        <ConfirmDialog
          ...
          loading={deleting}
          title="Hapus Edukasi"
          message={<>Yakin menghapus materi <span className="font-semibold">{targetDelete?.title}</span>? Tindakan ini tidak dapat dibatalkan.</>}
        />
      </div>
    </section>
  );
