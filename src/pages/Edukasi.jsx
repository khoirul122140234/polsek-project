...
/** Baris meta: Dilihat / Dibagikan */
function MetaRow({ item, className = "" }) {
  const { views, shares } = getNumbers(item);
  return (
...
function FeaturedCard({ item, onClick }) {
  return (
    <article className="grid grid-cols-1 overflow-hidden rounded-2xl bg-white shadow-md ring-1 ring-black/5 md:grid-cols-12">
      {/* ✅ aspect ratio biar aman di mobile/tablet */}
      ...
      <div className="p-4 sm:p-6 md:col-span-7">
        <p className="text-xs sm:text-sm text-gray-600">{formatDate(item.date)}</p>
        <h2 className="mt-1 line-clamp-2 text-lg sm:text-xl font-semibold leading-snug text-gray-900 md:text-2xl">
          {item.title}
        </h2>
        {item.excerpt && <p className="mt-2 line-clamp-3 text-sm sm:text-base text-gray-700">{item.excerpt}</p>}
  ...
  return (
    <article className="flex flex-col overflow-hidden rounded-2xl bg-white shadow-md ring-1 ring-black/5">
      ...
      <div className="p-4 sm:p-5">
        ...
        {item.excerpt && <p className="mt-2 line-clamp-3 text-xs sm:text-sm text-gray-700">{item.excerpt}</p>}
...
export default function Edukasi() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!alive) return;
        console.error(e);
        setErr(e?.message || "Gagal memuat daftar edukasi.");
        setRows([]);
      } finally {
  ...
  return (
    <section className="relative min-h-screen w-full bg-gray-50 pt-16">
      {/* ✅ BACKGROUND HITAM hanya di halaman utama edukasi */}
      ...
      <header className="relative w-full pt-0">
        <div className="w-full bg-gradient-to-r from-black via-zinc-800 to-slate-700 text-white shadow">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 py-6 md:py-8">
            <h1 className="text-2xl sm:text-3xl md:text-5xl font-extrabold tracking-tight">Edukasi</h1>
            <p className="mt-2 text-xs sm:text-sm md:text-base text-zinc-200">
              Menyediakan Informasi Hukum dan Keamanan untuk Masyarakat.
            </p>
          </div>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10 lg:px-8">
        <div className="mt-6 flex flex-col items-center gap-4">
          ...
          {/* ✅ tombol tab responsif: wrap + full width di mobile */}
        ...
        {loading ? (
          ...
          <>
            {showFeatured && featured && (
              <div className="mt-6">
                <FeaturedCard item={featured} onClick={() => navigate(`/edukasi/${featured.slug}`)} />
              </div>
            )}

            {/* ✅ FIX GRID UTAMA: tablet jangan dipaksa 3 kolom -> 3 kolom baru di lg */}
            <div className="mt-6 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {gridItems.map((it) => (
                <EduCard key={it.id} item={it} onClick={() => navigate(`/edukasi/${it.slug}`)} />
              ))}
            </div>

            {!err && filtered.length === 0 && !loading && (
              <div className="mt-8 rounded-xl border border-gray-200 bg-white p-8 text-center text-gray-600">
                Tidak ada edukasi yang cocok.
              </div>
            )}
...
export function EdukasiDetail() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const data = await get(`/edukasi/${slug}`);
        if (!alive) return;

        const images = Array.isArray(data?.images) ? data.images.filter(Boolean) : [];
        const main = data?.image || images[0] || "";

        setItem(data || null);
        setActiveImg(main);

        try {
          await post(`/edukasi/${slug}/view`, {});
          setItem((prev) => (prev ? { ...prev, viewCount: Number(prev.viewCount || 0) + 1 } : prev));
        } catch (_) {}
      } catch (e) {
        if (!alive) return;
        console.error(e);
        setErr(e?.message || "Gagal memuat detail edukasi.");
      } finally {
        if (alive) setLoading(false);
      }
    })();
  ...
  const handleShare = useCallback(async () => {
    try {
      ...
      const title = item?.title || "Edukasi";
      const text = "Cek informasi edukasi ini:";
      ...
      try {
        await post(`/edukasi/${slug}/share`, {});
        setItem((prev) => (prev ? { ...prev, shareCount: Number(prev.shareCount || 0) + 1 } : prev));
      } catch (_) {}
    } catch (e) {
      console.error(e);
    }
      ...
      <div className="mx-auto max-w-3xl px-4 py-6 sm:px-6 sm:py-10 lg:px-8">
        ...
        {loading ? (
          ...
          <article className="prose prose-slate max-w-none">
            <h1 className="mb-2 text-2xl sm:text-3xl font-extrabold text-gray-900 break-words">{item.title}</h1>
            <p className="text-xs sm:text-sm text-gray-600">{formatDate(item.date)}</p>

            <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <MetaRow item={item} className="mt-0" />
              <button
                onClick={handleShare}
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-full bg-yellow-400 px-4 py-2 text-sm font-semibold text-gray-900 shadow hover:bg-yellow-300"
              >
            ...
            <div className="mt-4 overflow-hidden rounded-2xl bg-white shadow ring-1 ring-black/5">
              <div className="relative aspect-[16/9]">
                <img
                  src={resolveImageUrl(activeImg || item.image)}
                  alt={item.title}
                  className="absolute inset-0 h-full w-full object-cover"
                  onError={(e) => (e.currentTarget.src = "/placeholder-wide.jpg")}
                />
                    ...
                    return (
                      <button
                        ...
                        className={`relative h-16 w-24 shrink-0 overflow-hidden rounded-lg ring-2 ${
                          active ? "ring-yellow-400" : "ring-transparent hover:ring-gray-200"
                        }`}
